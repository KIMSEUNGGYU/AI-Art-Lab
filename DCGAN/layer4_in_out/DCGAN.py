...
class DCGAN(object):
    ...
    def build_model(self):
        ## Creating a variable
        self.real_image = tf.placeholder(tf.float32, [64, 64, 64, 3], name = 'real_images')
    ...
    def train(self):
        ...
        for epoch in xrange(35):
            ...
            for index in xrange(0, batch_index):
                ...
                ## Update
                # Update D network
                _ = self.sess.run([discriminator_optimizer],
                                  feed_dict={ self.real_image: batch_images,
                                              self.random_noise: batch_random_noise })

                # Update G network
                _ = self.sess.run([generator_optimizer],
    ...
    def discriminator(self, input_, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            discriminator_layer1 = lrelu(conv2d(input_, 128, name='d_h0_conv'))                                         # (64, 64, 64, 3), 128  -> (?, 32, 32, 128)
            discriminator_layer2 = lrelu(batch_norm(conv2d(discriminator_layer1, 256, name='d_h1_conv'), 'd_bn1'))      # (?, 32, 32, 128), 256 -> (?, 16, 16, 256)
            discriminator_layer3 = lrelu(batch_norm(conv2d(discriminator_layer2, 512, name='d_h2_conv'), 'd_bn2'))      # (?, 16, 16, 256), 512 -> (?, 8, 8, 512)
            discriminator_layer4 = lrelu(batch_norm(conv2d(discriminator_layer3, 1024, name='d_h3_conv'), 'd_bn3'))     # (?, 8, 8, 512), 1024  -> (?, 4, 4, 1024)
            discriminator_output = linear(tf.reshape(discriminator_layer4, [64, -1]), 1, 'd_h4_lin')                    # (64, 1)
    ...
    def generator(self, random_noise):
        with tf.variable_scope("generator") as scope:
            ...
            generator_layer1 = deconv2d(generator_input, [64, 8, 8, 64 * 8], name='g_layer1')                           # (?, 8, 8, 512)
            generator_layer1 = tf.nn.relu(batch_norm(generator_layer1, 'g_bn1'))                                        # (?, 8, 8, 512)

            generator_layer2 = deconv2d(generator_layer1, [64, 16, 16, 64 * 4], name='g_layer2')                        # (?, 16, 16, 256)
            generator_layer2 = tf.nn.relu(batch_norm(generator_layer2, 'g_bn2'))                                        # (?, 16, 16, 256)

            generator_layer3 = deconv2d(generator_layer2, [64, 32, 32, 64 * 2], name='g_layer3')                        # (?, 32, 32, 128)
            generator_layer3 = tf.nn.relu(batch_norm(generator_layer3, 'g_bn3'))                                        # (?, 32, 32, 128)

            generator_output = deconv2d(generator_layer3, [64, 64, 64, 3], name='g_output')                             # (?, 64, 64, 3)
            generator_output = tf.nn.tanh(generator_output)                                                             # (?, 64, 64, 3)
    ...
    def sampler(self, random_noise):
        with tf.variable_scope("generator") as scope:
            ...
            sampler_layer1 = deconv2d(sampler_input, [64, 8, 8, 64 * 8], name='g_layer1')                               # (?, 8, 8, 512)
            sampler_layer1 = tf.nn.relu(batch_norm(sampler_layer1, 'g_bn1', train=False))                               # (?, 8, 8, 512)

            sampler_layer2 = deconv2d(sampler_layer1, [64, 16, 16, 64 * 4], name='g_layer2')                            # (?, 16, 16, 256)
            sampler_layer2 = tf.nn.relu(batch_norm(sampler_layer2, 'g_bn2', train=False))                               # (?, 16, 16, 256)

            sampler_layer3 = deconv2d(sampler_layer2, [64, 32, 32, 64 * 2], name='g_layer3')                            # (?, 32, 32, 128)
            sampler_layer3 = tf.nn.relu(batch_norm(sampler_layer3, 'g_bn3', train=False))                               # (?, 32, 32, 128)

            sampler_output = deconv2d(sampler_layer3, [64, 64, 64, 3], name='g_output')                                 # (?, 64, 64, 3)
            sampler_output = tf.nn.tanh(sampler_output)                                                                 # (?, 64, 64, 3)
